...
# Example usage
verts, faces, aux  = load_obj("mean_mesh.obj")  # Load the original mesh
...
W = np.load("svd_feet_E90_C80.npy")

print(W.shape)
W = torch.tensor(W, dtype=torch.float32)  # Add batch dimensionà
W = W.permute(1, 0)  # Change shape to [N, V, J+1]
print(W.shape, edges.shape)
W_new = subdivide_lbs_weights(W.squeeze(), edges)

W_new = W_new.permute(1, 0)  # Permute back to original shape [V, J+1]
print(W.shape, edges.shape)
np.save("svd_feet_E90_C80_sub.npy", W_new.cpu().numpy())  # Save as .npy file
