...
def loss_feet_calculate(mesh_f, mesh_t):
    """
    Calculate the loss based on the feet of the meshes.
    This function computes the distance between the feet vertices of two meshes.

    Parameters:
    - mesh_f: trimesh.Trimesh object for the predicted mesh
    - mesh_t: trimesh.Trimesh object for the target mesh

    Returns:
    - feet_loss: float, the computed loss based on feet vertices
    """
    # Load SMPLX body parts data from JSON file
    read_dictionary = np.load("utils/index_feet_3d_loss.npz", allow_pickle=True).item()
    index_feet = read_dictionary['idx']
    # faces_heel = read_dictionary['faces']
       
    # smplx_body_parts_path = os.path.join('data', 'SMPLX_BodyParts.json')
    # with open(smplx_body_parts_path, 'r') as file:
    #     smplx_body_parts = json.load(file)
    # left_foot_indices = smplx_body_parts['leftFoot'] 
    # right_foot_indices  = smplx_body_parts['rightFoot']

    vertices_left_foot = mesh_f.vertices[index_feet]
    # vertices_right_foot = mesh_f.vertices[right_foot_indices]

    mesh_left_foot = trimesh.Trimesh(vertices=vertices_left_foot, faces=None)
    # mesh_right_foot = trimesh.Trimesh(vertices=vertices_right_foot, faces=None)
    mesh_left_foot.export('left_foot.obj')
    # mesh_right_foot.export('right_foot.obj')
    exit()


    # Print the keys of the SMPLX body parts data
    print("Keys in SMPLX body parts data:", smplx_body_parts.keys())
    # Extract feet vertices from both meshes
    feet_f = mesh_f.vertices[mesh_f.vertices[:, 1] < 0.1]
    feet_t = mesh_t.vertices[mesh_t.vertices[:, 1] < 0.1]

    # Compute the distance between the feet vertices
    if len(feet_f) == 0 or len(feet_t) == 0:
        return float('inf')  # Return a large loss if no feet vertices are found

    feet_loss = np.mean(np.linalg.norm(feet_f - feet_t, axis=1))
...
if __name__ == "__main__":
    ...
    for (_, dirnames, _) in os.walk(abs_path_target_mesh):
        for j,dir in enumerate(dirnames):
            ...
            loss_feet = loss_feet_calculate(mesh_f, mesh_t)
