...
def recalage_rigide(model, optimizer0, scheduler0, betas, expression, body_pose, global_orient, left_hand_pose,
    ...
    with tqdm(total=total_iterations, desc="recalage_rigide", unit="iter") as pbar:
        for i in range(total_iterations):
            ...
            loss = chamfer_distance(output.vertices.to(device), pointcloud)[0] + MSE_loss(
...
def shape_optimization(step_shape_L_chamfer, step_shape_L_P2S, step_shape_L_sc, model, optimizer1, optimizer2,
    ...
    with tqdm(total=total_iterations, desc="shape optim step 1", unit="iter") as pbar:
        for i in range(total_iterations):
            ...
            smpl_mesh = Meshes(verts=[output.vertices.squeeze().to(device)],
                               faces=[(torch.tensor(model.faces.astype(np.float64), dtype=torch.int32)).to(device)])
...
def deformation_clothes(step_D_L_P2S, step_D_L_laplacien, step_D_L_normal, step_D_L_id, step_D_L_id_face, model,
                        ...
                        global_orient, left_hand_pose, right_hand_pose, t_params, pose_prior, pointcloud, body_mapping,
                        id_hand_twist, id_hand_coller, body_mapping1, path):
    ...
    with tqdm(total=total_iterations, desc="deformation vector step 1", unit="iter") as pbar:
        for i in range(total_iterations):
            ...
            smpl_mesh = Meshes(verts=[(output.vertices.squeeze()).to(device)],
                               faces=[(torch.tensor(model.faces.astype(np.float64), dtype=torch.int32)).to(device)])
            smpl_mesh_reg = Meshes(verts=[(output.vertices[0, idx_regularisation].squeeze()).to(device)],
                                   faces=[(torch.tensor(faces_regularisation, dtype=torch.int32)).to(device)])
    ...
    with tqdm(total=total_iterations, desc="deformation vector step 2", unit="iter") as pbar:
        for i in range(total_iterations):
            ...
            smpl_mesh = Meshes(verts=[(output.vertices.squeeze()).to(device)],
                               faces=[(torch.tensor(model.faces.astype(np.float64), dtype=torch.int32)).to(device)])
            smpl_mesh_reg = Meshes(verts=[output.vertices[0, idx_regularisation].squeeze()],
                                   faces=[(torch.tensor(faces_regularisation, dtype=torch.int32)).to(device)])
    ...
    with tqdm(total=total_iterations, desc="deformation vector step 3", unit="iter") as pbar:
        for i in range(total_iterations):
            ...
            if step_D_L_P2S:
                # loss_P2S = 0.0
                loss_P2S = knn_loss4(smpl_mesh, PC_full, idx_feet_complementaire)
            else:
                loss_P2S = 0.0
            ...
            if step_D_L_id_face:
                ...
                loss_id_face = 0.0
            # if step_D_L_P2S:
            #   loss_P2S = knn_loss4(smpl_mesh,PC1)
            # else:
            #   loss_P2S = 0.0
            loss = 2.5 * loss_P2S + 1e4 * loss_laplacian + 1e4 * loss_normal + loss_id + loss_id_face
    ...
    for (dirpath, dirnames, filenames) in walk(abs_path):
        ...
        for names in filenames:
            if names.endswith(".obj"):
                ...
                recalage_rigide(model, optimizer0, scheduler0, betas, expression, body_pose, global_orient,
                                left_hand_pose, right_hand_pose, t_params, pose_prior, pointcloud, body_mapping, path)
                if step_pose:
                    pose_optimization(step_pose_L_sc, model, optimizer, scheduler, betas, expression, body_pose,
                                      global_orient, left_hand_pose, right_hand_pose, t_params, pose_prior, pointcloud,
                                      body_mapping, path)

                shape_optimization(step_shape_L_chamfer, step_shape_L_P2S, step_shape_L_sc, model, optimizer1,
                                   optimizer2, scheduler1, scheduler2, betas, expression, body_pose, global_orient,
                                   left_hand_pose, right_hand_pose, t_params, pose_prior, pointcloud, PC, PC1,
                                   body_mapping, id_hand_twist, id_hand_coller, body_mapping1, path)
                mesh_f = deformation_clothes(step_D_L_P2S, step_D_L_laplacien, step_D_L_normal, step_D_L_id,
                                             step_D_L_id_face, model, optimizer4, optimizer5, scheduler4, scheduler5,
                                             PC1, D, betas, expression, body_pose, global_orient, left_hand_pose,
                                             right_hand_pose, t_params, pose_prior, pointcloud, body_mapping,
                                             id_hand_twist, id_hand_coller, body_mapping1, path)
                ...
                start_time = time.time()
                extract_texture_pifu(pifu_mesh, mesh_f, os.path.join(root_path, args.data_path, 'smplx_uv.obj'), path,
                                     uv_size=1024)
                # t_params,global_orient,left_hand_pose,right_hand_pose
                data = {'gender': gender, 'scale': s1_params, 'beta': betas, 'theta': body_pose,
                        'expression': expression, 'D': D, 't_params': t_params, 'global_orient': global_orient,
