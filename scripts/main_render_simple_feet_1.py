...
for (dirpath, dirnames,filenames) in walk(out_path_recons):
  for i,dir in enumerate(dirnames) :
    ...
    for val in range(1):
        ...
        theta1 = torch.tensor([math.pi*val/4])
    ...
    [-torch.sin(theta1), 0, torch.cos(theta1)]
        ...
        # mesh1=Meshes(verts=[(new_verts.to(device))], faces=[(torch.tensor(model.faces,dtype=torch.int64)).to(device)], textures=texture)
        mesh1=Meshes(verts=[new_verts.to(torch.float32).to(device)], faces=[(torch.tensor(model.faces.astype(np.int32),dtype=torch.int64)).to(device)], textures=texture)
        ...
        save_image(images[0,:,:,:3].permute((2,0,1)),out_path_val+'/'+dir[7:-4]+'.png')
