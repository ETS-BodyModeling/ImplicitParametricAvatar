...
#                         optimizer4, optimizer5, scheduler4, scheduler5, PC1, PC_full, D, betas, expression, body_pose,
#                         global_orient, left_hand_pose, right_hand_pose, t_param

def deformation_clothes(step_D_L_P2S, step_D_L_laplacien, step_D_L_normal, step_D_L_id, step_D_L_id_face, model, optimizer4, optimizer5, scheduler4, scheduler5, PC1, PC_full, D, betas, expression, body_pose, global_orient, left_hand_pose, right_hand_pose, t_params, pose_prior, pointcloud, body_mapping, id_hand_twist, id_hand_coller, body_mapping1, path, faces_simple, faces_simple_sub):
...
#               return_verts=True)
#   vv1, ff1 = trimesh.remesh.subdivide(output.vertices.squeeze().detach().cpu().numpy(), faces_simple)
...
#   vertice_torch = torch.tensor(mesh_sub.vertices, dtype=torch.float32).to(device)
  ...
  with tqdm(total=total_iterations, desc="deformation vector step 1", unit="iter") as pbar:
    for i in range(total_iterations):
      ...
      output = model(betas=betas, expression=expression,body_pose=body_pose,transl=t_params,global_orient=global_orient,left_hand_pose=left_hand_pose,right_hand_pose =right_hand_pose ,
                return_verts=True, D=D, faces_sub=faces_simple)
      
      smpl_mesh=Meshes(verts=[(output.vertices.squeeze())], faces=[(torch.tensor(faces_simple_sub.astype(np.float64),dtype=torch.int32)).to(device)])
      ...
      if step_D_L_normal:
        loss_normal=mesh_normal_consistency(smpl_mesh)
      else:
        loss_normal = 0.0
      ...
      if step_D_L_id:
        ...
        loss_id = 0.0
      if step_D_L_id_face:
        loss_id_face = 1e8 * torch.norm(torch.norm(D[idx_wo_fh], dim=1))
      else:
        loss_id_face = 0.0
      if step_D_L_P2S:
        loss_P2S = knn_loss_sub0(output.vertices.squeeze(),PC1)
      else:
        loss_P2S = 0.0

      loss = 2*loss_P2S +  5 * 1e4*loss_laplacian+   1e5*loss_normal + loss_id + loss_id_face

      # loss=2*loss_P2S  +    1e4*loss_laplacian+   1e4*loss_normal + loss_id + loss_id_face

      # loss= torch.norm(output.vertices)
  ...
  with tqdm(total=total_iterations, desc="deformation vector step 2", unit="iter") as pbar:
    for i in range(total_iterations):
      ...
      if step_D_L_id_face:
        ...
        loss_id_face = 0.0
      if step_D_L_P2S:
        loss_P2S = knn_loss_sub(output.vertices.squeeze(),PC1)
      else:
        loss_P2S = 0.0

      loss = 2*loss_P2S +  1e4*loss_laplacian+   1e4*loss_normal  + loss_id_face
  ...
  with tqdm(total=total_iterations, desc="deformation vector step 3", unit="iter") as pbar:
    for i in range(total_iterations):
      ...
      if step_D_L_id_face:
        ...
        loss_id_face = 0.0
      if step_D_L_P2S:
        loss_P2S = knn_loss_sub1(output.vertices.squeeze(),PC_full)
      else:
        loss_P2S = 0.0
  ...
  with tqdm(total=total_iterations, desc="deformation vector step 4", unit="iter") as pbar:
    for i in range(total_iterations):
      ...
      loss = 2* loss_P2S +  5*1e3*loss_laplacian+ 1e4*loss_normal + loss_id_face
      ...
      # scheduler4.step(loss_P2S)
  mesh_f = trimesh.Trimesh(vertices=(output.vertices.squeeze().detach().cpu().numpy()), faces=faces_simple_sub, process=False, maintain_order=True)
  mesh_f.export(path+ '/smpl_final_clothes_simple.obj')
  return mesh_f
...
def main_sample(abs_path, out_path, root_path, model_folder,
    ...
    D=torch.zeros((41738,3), dtype=torch.float32 , device=device, requires_grad=True)
    ...
    optimizer = torch.optim.Adam([t_params, body_pose, global_orient, expression], lr=1e-4)
    optimizer1 = torch.optim.Adam([t_params, body_pose, global_orient, betas, expression], lr=0.01)
    optimizer2 = torch.optim.Adam(
        [t_params, body_pose, global_orient, betas, left_hand_pose, right_hand_pose], lr=0.005)
    optimizer3 = torch.optim.Adam([body_pose, betas], lr=0.0001)



    optimizer4 = torch.optim.Adam([D], lr=5 * 1e-5)
    # optimizer4 = torch.optim.Adam([D], lr=1e-2)
    ...
    mesh_f = deformation_clothes(step_D_L_P2S, step_D_L_laplacien, step_D_L_normal, step_D_L_id, step_D_L_id_face,
                                 ...
                                 pointcloud, body_mapping, id_hand_twist, id_hand_coller, body_mapping1, out_path, mesh_simple.faces, mesh_simple_sub.faces)

    end_time = time.time()
    # Calculate the duration
    duration = end_time - start_time
    time_execution[3].append(duration)
    print(f"The process of deformation clothes took {duration} seconds.")
    logging.info(f"The process of deformation clothes took {duration} seconds.")
    # Log the duration to a file
    with open("time_log.txt", "a") as log_file:
...
if __name__ == "__main__":
    ...
    read_dictionary = np.load(os.path.join(root_path, args.data_path, 'index_sub_wo_fh.npy'), allow_pickle='TRUE').item()
    idx_wo_fh = read_dictionary['idx']
    ...
    # idx_feet_complementaire = sorted(set(range(10475)) - set(idx_feet.flatten().tolist()))
    # idx_feet_complementaire = sorted(set(range(10475)))
