...
def extract_texture_pifu(pifu,smpl_clothed,path_uv,path_save,uv_size=512):
  _, faces, aux = load_obj(path_uv)
  verts_uvs = aux.verts_uvs[None, ...]  # (1, V, 2)
  faces_uvs = faces.textures_idx[None, ...]  # (1, F, 3)
  verts_uvs_3d=torch.cat( (verts_uvs.squeeze(), torch.zeros((verts_uvs.shape[1],1)) ), 1 )

  mesh_uv = trimesh.Trimesh(vertices=np.asarray(verts_uvs_3d),faces=np.asarray(faces_uvs.squeeze()))
#   mesh_uv.export('uv.obj')
  ...
  bary_coor=trimesh.triangles.points_to_barycentric(verts_uvs_3d[faces_uvs[0,closest_tringles[2]]], points, method='cross')
  faces_matched=faces[0][closest_tringles[2]]


  vertices_smpl=smpl_clothed.vertices

  V0=np.concatenate( (np.multiply(vertices_smpl[faces_matched[:,0]][:,0], bary_coor[:,0]).reshape(-1,1),np.multiply(vertices_smpl[faces_matched[:,0]][:,1], bary_coor[:,0]).reshape(-1,1),np.multiply(vertices_smpl[faces_matched[:,0]][:,2], bary_coor[:,0]).reshape(-1,1)), axis=1)
  V1=np.concatenate( (np.multiply(vertices_smpl[faces_matched[:,1]][:,0], bary_coor[:,1]).reshape(-1,1),np.multiply(vertices_smpl[faces_matched[:,1]][:,1], bary_coor[:,1]).reshape(-1,1),np.multiply(vertices_smpl[faces_matched[:,1]][:,2], bary_coor[:,1]).reshape(-1,1)), axis=1)
  V2=np.concatenate( (np.multiply(vertices_smpl[faces_matched[:,2]][:,0], bary_coor[:,2]).reshape(-1,1),np.multiply(vertices_smpl[faces_matched[:,2]][:,1], bary_coor[:,2]).reshape(-1,1),np.multiply(vertices_smpl[faces_matched[:,2]][:,2], bary_coor[:,2]).reshape(-1,1)), axis=1)
  V=V0+V1+V2
