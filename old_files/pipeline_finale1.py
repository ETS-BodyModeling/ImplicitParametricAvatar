    ...
    for (dirpath, dirnames, filenames) in walk(abs_path):
      ...
      for names in filenames:
        if names.endswith(".obj"):
          ...
          # optimizer0 = torch.optim.Adam([t_params,global_orient,betas], lr=1e-3)
          # optimizer = torch.optim.Adam([t_params,body_pose,global_orient], lr=0.0001)
          # optimizer1 = torch.optim.Adam([t_params,body_pose,global_orient,betas,expression], lr=0.001) # it was 0.001
          # optimizer2 = torch.optim.Adam([t_params,body_pose,global_orient,betas,expression,left_hand_pose,right_hand_pose], lr=0.005) # it was 0.005
          # optimizer3 = torch.optim.Adam([body_pose,betas], lr=0.0001)
          # optimizer4 = torch.optim.Adam([D], lr=5*1e-5)
          # optimizer5 = torch.optim.Adam([D], lr=1e-4)
...
# change weigt MSE to 5
          ...
          for i in range(100):
            ...
            hand_penal=loss_with_constraint( left_hand_pose, -0.8, 0.5, 1e6) +torch.norm(left_hand_pose[0,id_hand_twist])*1e3 +torch.norm(left_hand_pose[0,id_hand_coller])*1e2  + torch.norm(right_hand_pose[0,id_hand_twist])*1e3 + torch.norm(right_hand_pose[0,id_hand_coller])*1e2
          ...
          for i in range(200):
            ...
            # loss_laplacian = mesh_laplacian_smoothing(smpl_mesh, method="uniform")
            # loss_normal=mesh_normal_consistency(smpl_mesh_reg)
            # reg_loss = 1e4*loss_laplacian+   1e4*loss_normal +  torch.norm(torch.norm(D[idx_D], dim=1)) + torch.norm(torch.norm(D, dim=1))
            # loss= 2*losss  +   reg_loss
            print(i,losss,loss)
            loss.backward()
            optimizer4.step()
          ...
          for i in range(1000):
            ...
            # loss_laplacian = mesh_laplacian_smoothing(smpl_mesh, method="uniform")
            # loss_normal=mesh_normal_consistency(smpl_mesh_reg)
            # # reg_loss = 1e1*loss_laplacian+   1e2*loss_normal +  1e6 * torch.norm(torch.norm(D[idx_D], dim=1)) + torch.norm(torch.norm(D, dim=1))
